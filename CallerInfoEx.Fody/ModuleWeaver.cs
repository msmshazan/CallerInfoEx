    ...
    public class ModuleWeaver : BaseModuleWeaver
    {
        public override void Execute()
        {
            if (Config != null)
            {
                ...
                var allinstructions = allmethods.ToDictionary(t => t, X => X.Body.Instructions.Where(x => x.OpCode == OpCodes.Callvirt && (x.Operand as MethodReference).Resolve().HasParameters).Where(x => (x.Operand as MethodReference).Resolve().DeclaringType.Namespace == FunctionNamespace).Where(x => (x.Operand as MethodReference).Resolve().Parameters.Last().HasCustomAttributes).Where(x => (x.Operand as MethodReference).Resolve().Parameters.Last().CustomAttributes.Any(p => p.AttributeType.Name == FunctionParameterAttribute )).Reverse());
                ...
                file.WriteLine(FunctionNamespace);
